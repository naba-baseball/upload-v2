defmodule UploadWeb.Admin.UploadLive do
  use UploadWeb, :live_view
  import UploadWeb.AdminComponents
  require Logger

  @impl true
  def mount(_params, _session, socket) do
    {:ok,
     socket
     |> allow_upload(:site_archive,
       accept: ~w(.gz),
       max_entries: 1,
       max_file_size: 524_288_000
     )}
  end

  @impl true
  def handle_event("validate", _params, socket) do
    {:noreply, socket}
  end

  @impl true
  def handle_event("save", params, socket) do
    dest =
      consume_uploaded_entries(socket, :site_archive, fn %{path: path}, entry ->
        dest = Path.join(System.tmp_dir!(), "site_archive_#{entry.uuid}.tar.gz")
        File.cp!(path, dest)
        {:ok, dest}
      end)

    Logger.info(
      event: "admin.save.reply",
      message: "saved upload to directory",
      upload_path: "#{dest}",
      socket: socket,
      params: params
    )

    {:noreply,
     socket
     |> put_flash(:info, "File uploaded successfully")
     |> push_navigate(to: ~p"/admin/upload")}
  end

  @impl true
  def render(assigns) do
    ~H"""
    <.admin_layout
      current_user={@current_user}
      flash={@flash}
      page_title="Upload Site Archive"
      active_tab="upload"
    >
      <p class="mb-6 text-gray-600 dark:text-gray-400">
        Upload a .tar.gz file containing the site assets.
      </p>

      <form
        id="upload-form"
        phx-submit="save"
        phx-change="validate"
        phx-drop-target={@uploads.site_archive.ref}
      >
        <div class="mb-4">
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
            Site Archive (.tar.gz)
          </label>
          <.live_file_input
            upload={@uploads.site_archive}
            class="block w-full text-sm text-gray-500 dark:text-gray-400
            file:mr-4 file:py-2 file:px-4
            file:rounded-full file:border-0
            file:text-sm file:font-semibold
            file:bg-indigo-50 dark:file:bg-indigo-900/50 file:text-indigo-700 dark:file:text-indigo-300
            hover:file:bg-indigo-100 dark:hover:file:bg-indigo-900
          "
          />
        </div>

        <section class="mb-4">
          <%= for entry <- @uploads.site_archive.entries do %>
            <.admin_card class="mb-2">
              <figure class="flex items-center justify-between mb-2">
                <span class="font-mono text-sm text-gray-900 dark:text-gray-100">
                  {entry.client_name}
                </span>
                <span class="text-sm text-gray-500 dark:text-gray-400">{entry.progress}%</span>
              </figure>

              <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
                <div
                  class="bg-indigo-600 dark:bg-indigo-500 h-2.5 rounded-full transition-all duration-300"
                  style={"width: #{entry.progress}%"}
                >
                </div>
              </div>

              <%= for err <- upload_errors(@uploads.site_archive, entry) do %>
                <p class="alert alert-danger text-red-600 dark:text-red-400 text-sm mt-1">
                  {error_to_string(err)}
                </p>
              <% end %>
            </.admin_card>
          <% end %>

          <%= for err <- upload_errors(@uploads.site_archive) do %>
            <p class="alert alert-danger text-red-600 dark:text-red-400 mb-2">
              {error_to_string(err)}
            </p>
          <% end %>
        </section>

        <.admin_button type="submit" phx-disable-with="Uploading...">
          Upload Archive
        </.admin_button>
      </form>
    </.admin_layout>
    """
  end

  defp error_to_string(:too_large), do: "Too large"
  defp error_to_string(:too_many_files), do: "You have selected too many files"
  defp error_to_string(:not_accepted), do: "You have selected an unacceptable file type"
end
